...
router.post('/edit', reqAuth, function(req, res) {
  ...
  Admin.find({_id: userID}).then((user) => {
    if (user.length == 1) {
      const query = {_id: user[0]._id};
      const newvalues = {$set: {name: name, email: email}};
      Admin.updateOne(query, newvalues, function(err, cb) {
        if (err) {
  ...
  if (errors.length > 0) {
    ...
    bcrypt.genSalt(10, (err, salt) => {
      bcrypt.hash(password, salt, null, (err, hash) => {
        ...
        Admin.updateOne(query, newvalues, function(err, usr) {
          if (err) {
...
router.post('/forgotpassword', (req, res) => {
  ...
  Admin.find({email: email}).then((user) => {
    if (user.length != 1) {
    ...
    if (errors.length > 0) {
      ...
      if (process.env.DEMO != 'yes') {
        ...
        transporter.sendMail({
          from: '"Roomnet" <' + smtpConf.auth.user + '>', // sender address
          to: email, // list of receivers
          subject: 'Roomnet Reset Password', // Subject line
          // eslint-disable-next-line max-len
          html: '<h1>Hey,</h1><br><p>If you want to reset your password, please click on the following link:</p><p><a href="' + '/auth/confirm-password/' + user._id + '">"' + '/auth/confirm-email/' + user._id + + '"</a><br><br>If you did not ask for it, please let us know immediately at <a href="mailto:' + smtpConf.auth.user + '">' + smtpConf.auth.user + '</a></p>', // html body
        });
        res.json({success: true});
      }
      ...
      bcrypt.genSalt(10, (err, salt) => {
        bcrypt.hash(password, salt, null, (err, hash) => {
          ...
          const query = {name: name, email: email,
            password: hash};
          Admin.create(query, function(err, user) {
            if (err) throw err;
            ...
            // send mail with defined transport object
              transporter.sendMail({
                from: '"Roomnet" <' + smtpConf.auth.user + '>',
                to: email, // list of receivers
                subject: 'Roomnet Confirm Account', // Subject line
                // eslint-disable-next-line max-len
                html: '<h1>Hey,</h1><br><p>Confirm your new account </p><p><a href="' + '/auth/confirm-email/' + user._id + '">"' + '/auth/confirm-email/' + user._id + '"</a><br><br>If you did not ask for it, please let us know immediately at <a href="mailto:' + smtpConf.auth.user + '">' + smtpConf.auth.user + '</a></p>', // html body
              });
              // eslint-disable-next-line max-len
