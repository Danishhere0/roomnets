...
const GallerySlider = ({ apartData }) => {
  ...
  useEffect(() => {
    ...
    if (apartData.media?.length > 0) {
      apartData.media.map((xxx) => newARRAY.push(xxx.uri));
    }
  ...
  /*useEffect(() => {
    const fetch = async (roomId) => {
      try {
        const res1  = await axios.get(`${process.env.API_URL}/getPostApartById`, {
          params: {
            postId: roomId
          },
        });
