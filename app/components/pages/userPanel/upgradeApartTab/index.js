...
await axios.get(`${process.env.API_URL}/${countryFull}`).then(rates=>{
 ...
 const config = {
    // public_key: process.env.REACT_APP_FLUTTERWAVE_PUBLIC_KEY,
      ...
      customer: {
        ...
        phonenumber: PostFlatReducer.mobile_number,
        name: PostFlatReducer.firstname,
      },
      customizations: {
        title: "RoomNets Ad Payment",
        description: "Payment Add Apartment Premimum",
        logo:
  ...
  const handleValidatePayment= React.useCallback(async(id,response,selected)=>{
    ...
    await axios.post(`${process.env.API_URL}/handleUpgradeApart`,params).then(res=>{
    if(res.data.status){
      router.push("/userpanel/PostApartmentSuccess");
    }
    }).catch(error=>{
      console.log(error)
    })
    ...
    await axios.post(`${process.env.API_URL}/handleUpgradesApartPayPal`,params).then(res=>{
...
export default upgradeApartTab;
